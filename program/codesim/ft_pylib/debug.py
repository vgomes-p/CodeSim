from .colors import BOLD, RED, GREEN, YLOW, PINK, DEFAULT

def debug(debug_text: str=False, stats: str="valid stats: success, failed or none",
    ...
    """
    valid_stats = ["success", "failed", "none"]
    debug_init = f"{YLOW}<-------------------------------------------------------------------->\nDEBUG:"
    debug_end = f"\n{YLOW}<-------------------------------------------------------------------->{DEFAULT}"
    oput = f"{PINK}Outputs:"
    error = f"{RED}Error:"
    ...
    if stats.lower() == "failed":
        ...
        return 0
    if stats.lower() == "none":
        print(debug_init, f"{BOLD}No process stats{YLOW}\n", error_text)
        if output:
            print(oput)
            for i in output:
                print(" ", i)
